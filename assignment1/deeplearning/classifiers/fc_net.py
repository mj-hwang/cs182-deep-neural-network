...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if self.use_batchnorm:
                X, cache = affine_bn_relu_forward(X, 
                                                  self.params["W%d"%i],
                                                  self.params["b%d"%i],
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            if self.use_batchnorm:
                dout, grads["W%d"%i], grads["b%d"%i], grads["gamma%d"%i], grads["beta%d"%i] = \
                    affine_bn_relu_backward(dout, caches[i-1])
            else:
                dout, grads["W%d"%i], grads["b%d"%i] = \
                    affine_relu_backward(dout, caches[i-1])

        loss += self.reg * 0.5 * sum([np.sum(self.params["W%d"%i]**2) for i in range(1, self.num_layers+1)])
