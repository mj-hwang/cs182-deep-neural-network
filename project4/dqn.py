...
class QLearner(object):
    ...
    def __init__(self,
        ...
        if not self.double_q:
            self.target_action = tf.reduce_max(target_q, axis=1)
        else:
            online_q_val = q_func(obs_tp1_float, self.num_actions, scope='q_func', reuse=True)
            online_action = tf.argmax(online_q_val, axis=1)
            self.target_action = tf.reduce_sum(target_q * tf.one_hot(online_action, self.num_actions), axis=1)
        
        y = self.rew_t_ph + (1-self.done_mask_ph) * gamma * tf.stop_gradient(self.target_action)
        y_online = tf.reduce_sum(q * tf.one_hot(self.act_t_ph, self.num_actions), axis=1)

        self.total_error = tf.reduce_mean(huber_loss(y-y_online))
    ...
    def step_env(self):
        ...
        # START OF YOUR CODE
        # ----------------------------------------------------------------------
        i = self.replay_buffer.store_frame(self.last_obs)
        latest_obs = np.expand_dims(self.replay_buffer.encode_recent_observation(), axis=0)

        # if the model is not intialized
        if not self.model_initialized:
            action = self.env.action_space.sample()

        # or if we choose greedy exploration
        elif self.exploration.value(self.t) > random.random():
            action = self.env.action_space.sample()

        # else
        else:
            action = self.session.run(self.action, 
                                      feed_dict={self.obs_t_ph: latest_obs})[0]
        
        obs, reward, done, info = self.env.step(action)
        
        self.replay_buffer.store_effect(i, action, reward, done)
        if done:
            obs = self.env.reset()
        self.last_obs = obs
        # ----------------------------------------------------------------------
    ...
    def update_model(self):
        ...
        if (self.t > self.learning_starts and \
            ...
            # 3.a
            obs, act, rew, obsp1, done = self.replay_buffer.sample(self.batch_size)

            # 3.b
            if not self.model_initialized:
                self.session.run(tf.variables_initializer(tf.global_variables()),
                                 {self.obs_t_ph:obs, 
                                  self.obs_tp1_ph:obsp1})
                self.model_initialized = True

            # 3.c
            self.session.run(self.train_fn, feed_dict={self.obs_t_ph:obs,
                                                       self.act_t_ph:act,
                                                       self.rew_t_ph:rew,
                                                       self.obs_tp1_ph:obsp1,
