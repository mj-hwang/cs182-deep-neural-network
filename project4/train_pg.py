...
class Agent(object):
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            # START OF YOUR CODE
            # ------------------------------------------------------------------
            N = tf.contrib.distributions.Normal(loc=sy_mean, scale=tf.exp(sy_logstd))
            sy_logprob_n = -N.log_prob(sy_ac_na)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
        return sy_logprob_n
    ...
    def sum_of_rewards(self, re_n):
        ...
        if not self.reward_to_go:
            for re in re_n:
                dc_r = np.array(re) @ np.power(self.gamma, range(len(re)))
                ret = dc_r * np.ones_like(re)
                q_n.append(ret)
        else:
            for re in re_n:
                ret = []
                for i in range(len(re)):
                    dc_r = np.array(re[i:]) @ np.power(self.gamma, range(len(re)-i))
                    ret.append(dc_r)
                q_n.append(ret)
