...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # backward
        dout, dW_vocab, db_vocab = temporal_affine_backward(dout, cache_temp_affine)
        if self.cell_type == 'rnn':
        ...
        dout, dW_proj, db_proj = affine_backward(dh0, cache_affine)
    ...
    def sample(self, features, max_length=30):
        ...
        word_curr = np.full((N, 1), self._start)
        for t in range(max_length):
            # (1)
            x, cache_embed = word_embedding_forward(word_curr, W_embed)
            
            # (2)
            if self.cell_type == 'rnn':
                h_curr, _ = rnn_step_forward(np.squeeze(x), h_prev, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                h_curr, c_curr, _ = lstm_step_forward(np.squeeze(x), h_prev, 
                                                      c_prev, Wx, Wh, b)

                # assign c_curr to c_prev for next step forward
                c_prev = c_curr
                
            # (3)    
            scores, _ = temporal_affine_forward(h_curr.reshape((N, 1, -1)), 
                                                W_vocab, 
                                                b_vocab)

            # (4) 
            word_curr = np.argmax(np.squeeze(scores), axis=1)
            captions[:, t] = word_curr

            # assign h_curr to h_prev for next step forward
            h_prev = h_curr        
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
